...
{unlisted}
I have a blog now! It’s a static site hosted using [Netlify](https://www.netlify.com). Rather than using an existing static site generator (SSG), I decided to create my own in vanilla JavaScript. I did this for several reasons. I try to always do something I’ve never done before each time I do a personal project. Writing a custom SSG would also make a good topic for my first post (which you’re reading!). And, of course, writing a bunch of code is a good excuse to delay writing actual blog posts.
...
After this exploration and briefly browsing [staticgen.com](https://www.staticgen.com), I decided to roll my own SSG.
...
- syntax highlighting
-->
At first, the build script only supported plain-text content. This is, of course, a terrible idea since it’s important to be able to use at least some formatting in blog posts. One option would be to create a new markup language, but that would lose me the battle-tested consistency of modern Markdown. Another option would have been to write in raw HTML but that’s less readable than Markdown, and I wanted to be able to write in [Ulysses](https://ulysses.app) (see [the section on writing](#writing-posts)). I had read about [remark](https://remark.js.org) before, so I decided to give it a shot. It was straightforward to set up and use, and I added a couple of plugins to do syntax highlighting and allow linking to specific headers.
...
## Writing Posts
<!--
- Ulysses is a great WYSIWYG editor for markdown files
...
-->
I wanted to write the posts in Ulysses, which I’ve been using for college notes and assignments in the past year. It’s a nice Markdown-adjacent sort-of-WYSIWYG editor. I added my `posts` directory as an External Folder in the sidebar, and view a list of all the posts there.
...
The other issue was with front matter. While there isn’t as big of an incompatibility with front matter as there was with paragraph spacing, Ulysses treats the front matter as prose rather than code. I briefly tried the front matter in a code block instead of the traditional `---`, but I realized that I might not actually need front matter. YAML is a simple-looking format that is actually very complex and potentially dangerous to parse. Plus it allows for a degree of customizability I don’t need. I only really needed four pieces of metadata about a blog post: the title, the publish date, the [slug](https://en.wikipedia.org/wiki/Clean_URL#Slug) (for its URL), and a Boolean flag indicating whether the post should be listed on the homepage and in the feed. I briefly tried making a simplified form of YAML but soon realized that there was a better, more human-readable way. Each article starts with a level 1 heading containing the date and title. Next, there is an optional line with “tags” surrounded by curly braces, like `{unlisted}`. Currently, there's just `{unlisted}` but I may add more in the future and the current design of the parsing code requires that each one have its own explicit syntax.
